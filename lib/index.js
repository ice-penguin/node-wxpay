...
var micropay = function(opt){
	...
	params.sign = wxUtil.objMd5UpperCase(params,apiKey);
...
var precreate = function(opt){
	...
	if(params.trade_type == "JSAPI"){
		if(state == "server"){
			...
			params.sub_appid = opt.sub_appid;
		}else{
 ...
 */
var payParams = function(package){
	opt = {
		package:package
	};
	opt.appId = appid;
	opt.timeStamp = new Date().getTime().toString();
	opt.nonceStr = wxUtil.randomString(20);
	opt.signType = "MD5"
	opt.paySign = wxUtil.objMd5UpperCase(opt,apiKey);
...
exports.initClient = function (params){
	...
	if(!params.apiKey){
		console.log("can't found apiKey. 缺少应用apiKey");
		return ;
	}
	if(!params.mch_id){
